...
export function Hero({
  ...
  return (
    <section
      className={cn(
        "relative isolate pt-10 pb-8 text-center sm:pt-16 sm:pb-12",
        className,
      )}
    >
      <div
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
        <span
          className="size-2.5 rounded-full bg-border-accented"
          aria-hidden
        />
        <span
          className="size-2.5 rounded-full bg-border-accented"
...
export interface LogoProps {
  ...
  label?: string;
  icon?: string;
  /** Path or URL of a brand mark, typically from `/public`. */
  src?: string;
  /** Invert the image in dark mode — for black marks such as Next.js. */
...
export function Stat({ children, value, label, className }: StatProps) {
  return (
    <div
      className={cn(
        "rounded-xl border border-border bg-muted/40 px-4 py-5 text-center",
        className,
      )}
    >
      {value && (
        <p className="text-lg font-semibold tracking-tight text-highlighted sm:text-xl">
          {value}
        </p>
      )}
